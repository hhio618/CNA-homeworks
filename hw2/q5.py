...
from base.degree_discount import degreeDiscountIC, spreadMaxFinder, runIC
import numpy as np
import community
import random
import itertools
...
def run(hw_name, seed_func):
    E = np.random.choice(data.load_actor_movie(),1000, replace=False)
    ...
    for percent in seed_percent:
            ...
            report += ["Find seed set for %d percent of graph(%d nodes)" % (
                int(percent*100), seed_size)]
            print(report[-1])

            # calculate initial set
            S = seed_func(G, seed_size, p)
            ...
            report += ['Avg. Targeted %d nodes out of %d' %
                       (target_count, len(G))]
            print(report[-1])
    ...
    plt.xlabel("#Initial")
    plt.ylabel("#Target")
    plt.savefig("outputs/%s/initial-target.png" % hw_name)
    # Find initial seed size for target of size 95 percent
    targeted_size = int(0.95*num_nodes)
    report += ["Searching for optimum seed size for target of size 95%%(%d of %d)" % (
        targeted_size, num_nodes)]
    print(report[-1])
    report += ["Searching using binary search..."]
    print(report[-1])
    S, Tsize = spreadMaxFinder(seed_func, G, targeted_size, step=1000, p=p)
    report += ["Find a solution #Seed: %d, #Target: %d" %
               (len(S), Tsize[len(S)])]
    print(report[-1])

    with open("outputs/%s/report.txt" % hw_name, "w") as f:
        f.write("\n".join(report))
        print("Report generated at (outputs/%s/report.txt)."%hw_name)


if __name__ == '__main__':
    run("q5", degreeDiscountIC)
